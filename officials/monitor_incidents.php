...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resolve'], $_POST['incident_id'], $_POST['resolution_notes'])) {
    ...
    if ($stmt->rowCount()) {
        ...
        $successMessage = "Incident #$incident_id has been marked as resolved.";
    } else {
...
<form method="get" class="row mb-3 g-2">
  <div class="col-md-3">
    <select name="category" class="form-select">
      ...
      <?php
        $categories = $incidentReport->getCategories();
        foreach ($categories as $cat) {
            $selected = ($filters['category'] == $cat['category_id']) ? 'selected' : '';
...
document.addEventListener('DOMContentLoaded', function () {
  const maps = document.querySelectorAll('[id^="map_"]');
  maps.forEach(div => {
